...
- steps: 解题步骤数组，每个 step 必须严格是如下结构：
  {
    "id": 1,
    "title": "步骤简短标题",
    "desc": "一句话描述",
    "explanation": "详细解析(HTML, 支持 <span class='math'> 和 <span class='highlight'>)",
    "actions": [ { "type": "...", ... }, ... ]
  }

⚠️ 关键约束（违反则渲染失败）：
- step 的 actions 字段**必须是数组**，即使只有一个 action 也要写成 [{...}]
- **禁止**把 action 的字段（type/id/points/of 等）直接平铺写在 step 上
- 错误示例 ❌：{ "type": "midpoint", "id": "E", "of": ["A","B"], "explanation": "..." }
- 正确示例 ✅：{ "id": 1, "title": "取中点", "desc": "...", "explanation": "...", "actions": [{ "type": "midpoint", "id": "E", "of": ["A","B"] }] }
...
4. **explanation 字段**用 HTML，严禁 LaTeX：
   - 数学表达式用 <span class='math'>△ABC</span>，不要用 \\(\\triangle ABC\\) 或 \\triangle ABC
   - 重点结论用 <span class='highlight'>
   - 分数直接写 "1/2" 或用 HTML 标签 <sup>/<sub>，不要用 \\frac{1}{2}
   - 字符串内不能有裸反斜杠、裸换行（JSON 规范），必要时分成多句用句号连接
